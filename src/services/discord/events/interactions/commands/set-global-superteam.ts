...
export default class SetGlobalSuperteamCommand implements CommandHandler {
  constructor(private readonly channelCache: ChannelCache) {}

  name = 'set-global-superteam';
  commandBuilder = new SlashCommandBuilder()
    .setName(this.name)
    .setDescription('Set a channel to become a Global Superteam Channel.')
  ...
  process = async (interaction: ChatInputCommandInteraction<'cached'>) => {
    ...
    if (
      ...
      await interaction.followUp({
        embeds: [
          EmbedUtils.Warning(
            'This channel is already set as a global channel with access to Superteam.',
          ),
        ],
        ephemeral: true,
      });
    ...
    await interaction.followUp({
      embeds: [
        EmbedUtils.Success(
          `The channel ${channelMention(selectedChannel.id)} has been set as a global channel with access to Superteam.`,
        ),
      ],
      components: guildData.tag ? [] : [globalTagButton()],
      ephemeral: true,
